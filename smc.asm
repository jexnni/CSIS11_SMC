...
START LD R6, PTR ; Initialize stack pointer
        LEA R0, PROMPT1
        PUTS
        JSR READ_INPUT 
        JSR PUSH_STACK
...
; Reads 4 digits from the user and pushes each digit
READ_INPUT
        AND R0, R0, #0 
        AND R2, R2, #0
...
; Converts the 4 digits from the stack into a complete number
CALCULATE_INPUT
        AND R0, R0, #0
        LD R3, PLACE_1000

        JSR POP_STACK ; Get thousands 
        ADD R1, R0, #0
        JSR MULTIPLY
        ADD R0, R0, R1
        LD R3, PLACE_100

        JSR POP_STACK ; Get hundreds 
        ADD R1, R0, #0
        JSR MULTIPLY
        ADD R0, R0, R1
        LD R3, PLACE_10

        JSR POP_STACK ; Get tens 
        ADD R1, R0, #0
        JSR MULTIPLY
        ADD R0, R0, R1
        LD R3, PLACE_1

        JSR POP_STACK ; Get ones 
        ADD R1, R0, #0
        JSR MULTIPLY
        ADD R0, R0, R1
        RET
...
DIVISION
        ...
        BR DIV_LOOP
        DIV_DONE
        ADD R5, R1, #0 ; Store remainder in R5
        JSR PRINT_RESULT
        ADD R0, R5, #0
        JSR PRINT_RESULT
        RET
